...
const hiddenAddressStatuses: BookingStatus[] = ['REQUESTED', 'GARDENER_ASSIGNED', 'REJECTED', 'CANCELLED'];
...
export class TrackingService {
  ...
  private async job(db: Prisma.TransactionClient, userId: string, id: string): Promise<{ job: TrackingJob; customer: boolean }> {
    const job = await db.serviceBooking.findUnique({ where: { id }, include: { gardener: true, service: true } });
  ...
  async read(userId: string, id: string, asGardener = false): Promise<TrackingResponse> {
    ...
    const { job } = access;
    if (asGardener && (job.gardener?.userId !== userId || !job.gardener.active)) throw new ForbiddenException();
    const customer = access.customer && !asGardener;
    const permitted = customer || !hiddenAddressStatuses.includes(job.status);
    const address = permitted ? await this.db.address.findFirst({ where: { id: job.addressId, userId: job.userId }, select: { fullAddress: true, postalCode: true } }) : null;
    const records = permitted ? await this.db.bookingActivity.findMany({ where: { bookingId: id, kind: { in: privateTrackingKinds } } }) : [];
    const location = records.find((row) => row.kind === 'TRACKING_LOCATION');
    const live = job.status === 'ON_THE_WAY' && !!location && Date.now() - location.createdAt.getTime() < 45000;
    const destination = records.find((row) => row.kind === 'TRACKING_DESTINATION');
    return {
      id, viewer: customer ? 'CUSTOMER' : 'GARDENER', status: job.status,
      service: job.service.title, scheduledAt: job.scheduledAt,
      gardenerName: job.gardener?.name ?? null, address,
      location: live && location ? location.data : null,
      locationUpdatedAt: job.status === 'ON_THE_WAY' ? location?.createdAt ?? null : null,
      live, destination: destination?.data ?? null,
    };
  }

  async write(userId: string, id: string, point: TrackingPointDto, destination = false): Promise<{ saved: boolean }> {
    return this.db.$transaction(async (tx) => {
      ...
      if (destination ? ['COMPLETED', 'CUSTOMER_CONFIRMED', 'OUTCOME_RECORDED', 'CANCELLED', 'REJECTED', 'NO_SHOW'].includes(job.status) : job.status !== 'ON_THE_WAY')
      ...
      const data = { latitude: point.latitude, longitude: point.longitude, accuracy: point.accuracy ?? null };
      await tx.bookingActivity.upsert({
        where: { bookingId_actorId_requestId: { bookingId: id, actorId: userId, requestId } },
        create: { bookingId: id, actorId: userId, requestId, kind, data }, update: { data, createdAt: new Date() },
      });
      return { saved: true };
    });
  }
  ...
  async stop(userId: string, id: string): Promise<{ stopped: boolean }> {
    const { job } = await this.job(this.db, userId, id);
    if (job.gardener?.userId !== userId) throw new ForbiddenException();
    await this.db.bookingActivity.deleteMany({ where: { bookingId: id, actorId: userId, kind: 'TRACKING_LOCATION' } });
    return { stopped: true };
  }
}
